(() => {
  const STYLE_ID = "su-ecosystem-unified-style-v4";

  function apply() {
    const panel = document.getElementById("su-loto-cloud-panel");
    const card = panel?.querySelector(".su-loto-cloud-card");
    ...
    if (!document.getElementById(STYLE_ID)) {
      ...
      style.textContent = `
        ...
        .su-loto-cloud-panel .su-loto-cloud-card{width:min(520px,100%)!important;max-height:none!important;margin:auto 0!important;border-radius:28px!important;padding:0!important;overflow:visible!important;box-shadow:0 28px 80px #0008!important;background:#fff!important}
        ...
        .su-loto-grid span{font-size:.78rem!important;text-transform:uppercase;letter-spacing:.04em}.su-loto-grid strong{font-size:.98rem;overflow-wrap:anywhere}.su-loto-actions{display:grid!important;grid-template-columns:1fr 1fr!important;gap:10px!important;padding-bottom:max(8px,env(safe-area-inset-bottom))}.su-loto-actions button{width:100%!important;min-height:46px!important;padding:12px 14px!important;background:#f3edf6!important;color:#3d1748!important;border:1px solid #e2d4e8!important;font-size:.96rem!important}
    ...
    if (card.dataset.unifiedV4) return true;
    card.dataset.unifiedV4 = "true";

    const close = document.getElementById("su-loto-cloud-close");
    const account = document.getElementById("su-loto-cloud-account");
    const grid = card.querySelector(".su-loto-cloud-grid");
    const actions = card.querySelector(".su-loto-cloud-actions");
    if (grid) grid.classList.add("su-loto-grid");
    if (actions) actions.classList.add("su-loto-actions");
    ...
    if (account) {
      new MutationObserver(() => {
        ...
        if (copy) copy.textContent = account.textContent;
      }).observe(account, { childList: true, characterData: true, subtree: true });
    }
    return true;
  }
