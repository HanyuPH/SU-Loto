(() => {
  ...
  const EXPECTED_NUMBERS = 15;
  const MAX_NUMBER = 25;
  const MIN_HITS = 11;
  const MAX_HITS = 15;
  const CONTESTS_API = "SULotoContests";
  const ui = {};
  ...
  function buildInterface() {
    ...
    section.innerHTML = `
      <div class="official-sync-head"><div>
        <p class="eyebrow purple">Resultado oficial automático</p>
      ...
      <div class="official-specific-search">
        <label><span>Buscar concurso específico</span>
          <input id="official-contest-number" type="number" min="1" inputmode="numeric" placeholder="Ex.: 3751">
        </label>
  ...
  function openContest(number) {
    switchToContests();
    setTimeout(() => {
      const button = [...document.querySelectorAll(".history-open")]
        .find(item => item.getAttribute("aria-label") === `Abrir concurso ${number}`);
      button?.click();
    }, 50);
  }
  ...
  function registerPreview() {
    ...
    if (ui["official-update-dot"]) ui["official-update-dot"].hidden = true;
    setState("success", `Concurso ${preview.number} registrado`, "A carteira foi conferida automaticamente.");
    openContest(preview.number);
  }
